...
struct Ipv4 dataLinkUnpack(struct PPP ppp){
    printf("----------数据链路层开始解封装----------\n");
    //还原数据部分
    const unsigned char* data=(unsigned char *)(&(ppp.ip));
    int reduceLength= lengthAfterReduce(data,ppp.extendLength);
    unsigned char* ans= byteReduce(data,ppp.extendLength,reduceLength);
    //进行循环冗余检验
    //新建一个数组带有FCS
